...
must_haves:
  truths:
    ...
    - "Inbound messages are routed based on connection autonomy: Full Manual queues for review, Full Auto delivers immediately"
...
2. Build DeliveryConfirm payload with messageId bytes, signature, timestamp, state="delivered"
...
interface RoutedMessage {
  ...
  state: string; // resulting state after routing
}
```
...
2. If connection not found or not active, set state to "rejected" (message from unknown sender)
3. If autonomyLevel === "full_manual":
   - Update message state to "pending_human_review" via messageStore.updateState
...
4. If autonomyLevel === "full_auto":
   - Update message state to "delivered" via messageStore.updateState
   - Return with state "delivered" (caller presents to agent)
5. Default (future autonomy levels): treat as full_manual

Also export a `getPendingForReview(): MessageRecord[]` method that returns all messages with state "pending_human_review", ordered by created_at ASC (oldest first). This powers the HEARTBEAT.md checklist.

`approveMessage(messageId: string, action: 'agent_handle' | 'human_respond'): void`
- Update message state from "pending_human_review" to "approved"
...
- Full Manual connection routes message to pending_human_review state
- Full Auto connection routes message to delivered state
- Unknown sender gets rejected state
- getPendingForReview returns only pending messages
- approveMessage transitions from pending_human_review to approved
...
- (Reconnection is hard to unit test -- integration test in Plan 04 will validate)
  ...
  <done>InboundRouter dispatches inbound messages to pending_human_review (Full Manual) or delivered (Full Auto) based on connection autonomy level. getPendingForReview surfaces messages for HEARTBEAT.md. RelayClient supports auto-reconnection with exponential backoff and jitter. Multiple envelope handlers coexist. All tests pass.</done>
</task>
...
- Full Manual messages are queued with pending_human_review state
- Full Auto messages are marked delivered immediately
