...
must_haves:
  truths:
    ...
    - "New connections default to full_manual autonomy level"
...
<task type="auto">
  ...
  <action>
    **Create `skill/src/connection-store.ts`:**
    ...
    - `setAutonomy(peerAddress: string, level: AutonomyLevel): Connection` -- Changes autonomy level. Per locked decision: downgrade from full_auto to full_manual takes effect immediately. The store just records the change; enforcement is at the message processing layer (Phase 3+).
    ...
    **Create `skill/src/connection-store.test.ts`:**
    ...
    - setAutonomy changes autonomy level
  ...
  <done>ConnectionStore persists connections as JSON with all states, autonomy levels, nicknames, and TTL expiration. Default autonomy is full_manual. Short message limit enforced at 280 chars. Sorting follows state priority then recency. Save/load roundtrip preserves all data. All connection store tests pass.</done>
</task>
...
- Autonomy levels configurable per connection (AUTO-01)
